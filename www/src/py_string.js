...
var signed_hex_format = function(val, upper, flags) {
    ...
    if (flags.alternate) {
        if (ret.charAt(0) === "-") {
            if (upper) {ret = "-0X" + ret.slice(1)} else {ret = "-0x" + ret.slice(1)}
        } else {
            if (upper) {ret = "0X" + ret} else {ret = "0x" + ret}
        }
    }
...
var octal_format = function(val, flags) {
    ...
    if (flags.alternate) {
        if (ret.charAt(0) === "-") {ret = "-0o" + ret.slice(1)} else {ret = "0o" + ret}
    }
    ...
    while (pos < _len) {
        ...
        if (car == "{" && s.charAt(pos + 1) == "{") {
            ...
            var end = pos + 1,
                nb = 1
            while (end < _len) {
                if (s.charAt(end) == "{") {nb++; end++} else if (s.charAt(end) == "}") {
                    nb--; end++
                    if (nb == 0) {
...
str_funcs.lstrip = function(self) {
    var $ = $B.args("lstrip", 2, {self: null, chars: null},
...
str_funcs.rjust = function() {
    var $ = $B.args("rjust", 3, {self: null, width: null, fillchar: null},
                arguments, {fillchar: " "}, null, null),
        _self = to_string($.self)
    var len = str.mp_length(_self)
    if ($.width <= len) {
...
str_funcs.rstrip = function() {
    var $ = $B.args("rstrip", 2, {self: null, chars: null},
                arguments, {chars: _b_.None}, null, null)
    var chars = $.chars,
        _self = to_string($.self)
    if (chars === _b_.None) {
...
str_funcs.strip = function() {
    var $ = $B.args("strip", 2, {self: null, chars: null},
                arguments, {chars: _b_.None}, null, null)
    var _self = to_string($.self)
    if ($.chars === _b_.None) {
        return _self.trim()
...
str_funcs.title = function(self) {
    ...
    for (var char of _self) {
        ...
        if ($B.in_unicode_category('Ll', cp) ||
                ...
                $B.in_unicode_category('Lt', cp)) {
            if (! state) {
                if (Object.hasOwn($B.unicode_titles, char)) {
                    res += $B.unicode_titles[char]
                } else {
                    res += char.toUpperCase()
                }
                state = "word"
